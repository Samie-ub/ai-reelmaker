...
function ReelSceneFrame({ scene, templateId, startFrame, totalFrames }: { scene: ReelScene; templateId: ReelProject['templateId']; startFrame: number; totalFrames: number }) {
  ...
  return (
    <AbsoluteFill style={{ backgroundColor: scene.background, color: scene.textColor, fontFamily: 'Inter, Arial, sans-serif', padding: 84, justifyContent: templateId === 'metric' ? 'space-between' : 'center', textAlign: scene.alignment }}>
      ...
      {templateId === 'metric' && <div style={{ fontSize: 27, fontWeight: 600, letterSpacing: 3, textTransform: 'uppercase', color: scene.secondaryTextColor }}>Proof / scene</div>}
      <div style={{ ...motion, opacity: motion.opacity * exit }}>
        ...
        <p style={{ fontSize: 34, lineHeight: 1.35, margin: '48px 0 0', color: scene.secondaryTextColor }}>{scene.subtitle}</p>
      </div>
      {templateId === 'metric' && <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 24, color: scene.secondaryTextColor }}><span>REELMAKER</span><span>9:16</span></div>}
