...
describe('Ollama reel generator', () => {
  it('requests a schema-constrained suggestion from llama3.2', async () => {
    ...
    expect(body.format.required).toEqual(['title', 'subtitle', 'accent', 'alignment', 'duration']);
  ...
  it('rejects a response outside the safe reel schema', async () => {
    const fetcher = vi.fn<typeof fetch>().mockResolvedValue(ollamaResponse({ ...validSuggestion, duration: 90 }));
